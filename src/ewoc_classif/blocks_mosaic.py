...
def blocks_mosaic(
    ...
    if ret == 0:
        ...
        if not is_empty_dirs(cogs_dirpath / f"{tile_id}/{year}_{season}"):
            ...
            logger.debug(f"Try to upload files for {tile_id} to {root_s3}")
            try:
                nb_prd, __unused, up_dir = EWOCPRDBucket().upload_ewoc_prd(
                    cogs_dirpath,
                    production_id)
            except UploadProductError:
                msg= f'Upload from {cogs_dirpath} to {root_s3} failed!'
                logger.error(msg)
                # TODO: remove this message used by Alex
                print(msg)
                proc_status = False
            ...
            if stac_paths:
                ...
                logger.error('No STAC files were found to start ingestion')
        else:
...
def run_block_mosaic(
    ...
    if not proc_status:
        raise RuntimeError(f"Blocks processing of {tile_id} failed!")
    if clean:
